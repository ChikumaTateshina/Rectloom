...
namespace Rectloom.Core.Css.Values
{
    ...
    /// </summary>
    /// <remarks>
    /// The full standard list is included rather than a popular subset. A partial table turns a
    /// perfectly valid stylesheet into an "invalid value" diagnostic, which is far more confusing
    /// than the size of the table is costly.
    /// </remarks>
    public static class CssNamedColors
    {
        private static readonly Dictionary<string, uint> Rgb = new Dictionary<string, uint>(StringComparer.Ordinal)
        {
        ...
        /// <summary>The CSS keyword for a fully transparent colour.</summary>
        public const string Transparent = "transparent";

        /// <summary>Number of named colours in the table, excluding <see cref="Transparent"/>.</summary>
        public static int Count => Rgb.Count;

        /// <summary>
        /// Looks up a named colour.
        /// </summary>
        /// <param name="name">Colour name. Case is ignored, as CSS keywords are case insensitive.</param>
        /// <param name="rgb">
        /// The colour packed as <c>0xRRGGBB</c> when found. <see cref="Transparent"/> is not part of
        /// this table because it also sets alpha.
        /// </param>
        /// <returns><see langword="true"/> when the name is a CSS named colour.</returns>
        public static bool TryGetRgb(string name, out uint rgb)
        {
            ...
            return Rgb.TryGetValue(name.ToLowerInvariant(), out rgb);
        }
    }
}
